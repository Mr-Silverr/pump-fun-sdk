/**
 * PumpFun All-Claims Bot - GitHub API Client
 *
 * Resolves the developer behind a social fee claim: the numeric GitHub user id
 * carried by the claim event, and the repo linked from the token's own metadata.
 *
 * Works unauthenticated (60 requests/hour). Set GITHUB_TOKEN to raise that to
 * 5,000/hour. Only instant claims are enriched, so the unauthenticated budget
 * is sufficient at normal claim volume.
 */
