/**
 * PumpFun Claim Bot - RPC Connection Manager with Fallback
 *
 ...
 *
 * Shared implementation with the channel and all-claims bots.
 */
...
export class RpcFallback {
    ...
    constructor(urls: string[], config: ConnectionConfig = { commitment: 'confirmed' }) {
        ...
        // Reject malformed endpoints at construction. Without this an unset env
        // var reaches web3.js as `undefined` and every poll fails with an opaque
        // "Endpoint URL must start with http:" for the life of the process.
        const valid: string[] = [];
        for (const url of urls) {
            if (typeof url !== 'string' || url.trim() === '') continue;
            if (!/^https?:\/\//i.test(url)) {
                throw new Error(`RPC URL must start with http:// or https://, got: ${maskUrl(url)}`);
            }
            valid.push(url.trim());
        }
        if (valid.length === 0) {
            throw new Error('At least one valid RPC URL is required (all provided entries were empty)');
        }
        this.urls = valid;
    ...
    /**
     * Rotate immediately, without waiting for MAX_CONSECUTIVE_FAILS.
     *
     * Used when a failure is not a call failure: a WebSocket that connects and
     * then delivers no notifications never produces an RPC error to count.
     */
    rotateNow(): void {
