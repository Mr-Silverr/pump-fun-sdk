...
> Key terms and concepts used throughout the Pump SDK documentation, from protocol mechanics to SDK and infrastructure vocabulary.
...
| Term | Definition |
|------|-----------|
| **Fee Tier** | The fee rates applied to trades, selected by the token's current market cap from the on-chain `FeeConfig.feeTiers` table. Lower market cap tiers generally carry higher fees. |
...
| **Cashback** | SOL returned to traders on cashback-enabled tokens (the `cashback` flag at creation; `isCashbackCoin` on the curve/pool) as a trading incentive. |
| **CTO (Creator Takeover)** | When a token's creator address changes on-chain (a `set_creator` event), redirecting future creator fees to a different wallet. |
| **Distributable Fee** | The SOL currently available in a creator vault for distribution to shareholders; distribution requires it to exceed an on-chain minimum. |
...
| **OnlinePumpSdk** | Wraps `PUMP_SDK` with RPC fetchers, quotes, and routed trading. Reads on-chain state, then builds instructions. Requires a `Connection`. |
...
| **SPL Token** | Solana Program Library Token: the standard token program on Solana. Tokens created via `create_v2` use the newer Token-2022 program. |
...
| Term | Definition |
|------|-----------|
| **MCP (Model Context Protocol)** | A protocol for connecting AI assistants to external tools and data sources. The MCP server exposes the Pump SDK as tools (67 at last count) to Claude, Copilot, and other assistants. |
...
| **PumpOS** | The web-based desktop environment (in `website/`) for interacting with the Pump ecosystem. |
...
| **i18n** | Internationalization: the DeFi agents support multiple languages, auto-translated via OpenAI. |
